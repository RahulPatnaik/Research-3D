...
class GRPOConfig:
    ...
    base_model_name: str = "meta-llama/Llama-3.2-1B-Instruct"  # Base model for LoRA
    world_dim: int = 20
    max_bricks: int = 100
...
class GRPOTrainer:
    ...
    def _setup_model(self):
        """Initialize the BrickGPT model and tokenizer with LoRA support."""
        try:
            ...
            self.logger.info(f"Loaded model directly from {self.config.model_name_or_path}")
            
        except Exception as e:
            self.logger.warning(f"Failed to load model directly: {e}")
            self.logger.info("Trying to load as LoRA adapter...")
            ...
            try:
                ...
                # Load LoRA adapter
                self.model = PeftModel.from_pretrained(base_model, self.config.model_name_or_path)
                self.model = self.model.to(self.device)
                
                self.logger.info(f"Loaded LoRA adapter from {self.config.model_name_or_path} on base model {self.config.base_model_name}")
                
            except Exception as e2:
                self.logger.error(f"Failed to load LoRA adapter: {e2}")
                # Fallback to a working model
                self.logger.info("Falling back to a working model...")
                ...
                self.logger.info("Using fallback model: microsoft/DialoGPT-medium")
        ...
        try:
            ...
            self.logger.warning(f"Failed to initialize BrickGPT: {e}")
            self.logger.info("Will use direct model generation instead")
            self.brickgpt = None
    ...
    def _generate_sequence_group(self, captions: List[str]) -> List[Dict[str, Any]]:
        ...
        for caption in captions:
            ...
            for _ in range(self.config.group_size):
                try:
                    if self.brickgpt is not None:
                        # Use BrickGPT for generation
                        ...
                        generated_text = self.tokenizer.decode(outputs[0][prompt.shape[0]:], skip_special_tokens=True)
                        bricks_text = generated_text
                        
                        # Try to parse as brick structure
                        try:
                            brick_lines = [line.strip() for line in bricks_text.split('\n') if line.strip()]
                    ...
                    sequences.append({
                        "caption": caption,
                        "bricks": bricks if 'bricks' in locals() else BrickStructure([]),
                        "bricks_text": bricks_text,
                        "reward": reward,
                        "length": len(bricks) if 'bricks' in locals() else 0
                    })
                except Exception as e:
